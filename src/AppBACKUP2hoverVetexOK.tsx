...
import { 
  ...
  Plane, Ray, IntersectionTests, Cartesian2 // <--- ADĂUGAT AICI
} from 'cesium';
import { 
...
// 1. Adaugă acest import pentru Drag & Drop
...
import { 
    ...
    IconDownload, IconUpload, IconGripVertical, IconMessageCircle // <--- ADĂUGAT AICI
...
interface MapEventsProps {
    ...
    onDoubleClick: () => void;
    onMouseMoveGhost: (cartesian: Cartesian3, movementPosition?: { x: number, y: number }) => void; // <--- UPDATE AICI
    onSelectionChange: (id: number | null) => void;
    onVertexSelect?: (index: number) => void; // <--- NOU
}
...
// CLICK
    handler.setInputAction((movement: any) => {
      ...
      // --- LOGICA CLICK PE VERTEX HANDLE (CORECȚIE) ---
      // FIX: Accesăm "pickedObject.id.id" pentru a obține string-ul ID (ex: "vertex_handle_0")
      // "pickedObject.id" este obiectul Entity, nu string-ul direct.
      ...
      if (!drawActive && !relocateActive && !isEditingVertices) {
          if (defined(pickedObject) && pickedObject.id) {
              // Fallback sigur: uneori poate fi string direct (primitive), alteori Entity object
...
// FIX: Acceptăm DOAR poziții 3D valide (Teren sau Tileset).
      // Am eliminat complet fallback-ul la 'pickEllipsoid'.
    ...
    // MOUSE MOVE
    handler.setInputAction((movement: any) => {
        // A. HOVER (Doar dacă nu suntem în mod editare/desenare)
        if (!drawActive && !relocateActive && !isEditingVertices) {
            ...
            if (defined(pickedObject) && pickedObject.id) {
                ...
                // Excludem Vertex Handles din Hover-ul normal
        ...
        // B. GHOST & SNAP
// B. GHOST & SNAP (FIX FINAL: Corecție nume variabilă)
        if (relocateActive || isEditingVertices) {
            // 1. Încercăm să obținem poziția 3D (pentru Relocare sau Snap)
            const picked3D = viewer.scene.pickPosition(movement.endPosition);
            
            let targetPos = picked3D; // Poate fi undefined (dacă e pe cer)

            // 2. Logică Snap (doar dacă avem poziție 3D validă)
            if (defined(targetPos) && isSnappingEnabled) {
                const snapped = calculateSnap(targetPos);
                if (snapped) targetPos = snapped;
            }

            // 3. Apelăm funcția de procesare
            if (relocateActive && targetPos) {
                // ATENȚIE: Aici folosim 'relocateActive' (prop-ul), nu 'isRelocating'
                onMouseMoveGhost(targetPos, movement.endPosition);
            } 
            else if (isEditingVertices) {
                // Pentru Vertex Edit trimitem un punct dummy dacă suntem pe cer
...
function App() {
  ...
  const dragPlaneRef = useRef<Plane | null>(null); // <--- ADĂUGAT AICI
  ...
  const fileInputRef = useRef<HTMLInputElement>(null); // Referință pentru upload fișier
...
const handleCreateLayer = async () => {
      ...
      const defaultStyle = newLayerType === 'POINT' ? { color: COLORS.orange, pixelSize: 15 } 
          ...
          : { color: '#FF0000' }; // Default style pentru COMMENT
...
// --- HELPER 2: Logică PURĂ 3D pentru Editare Vertex (Fix Problema 2D) ---
...
const updateGhostForVertex = (movementPosition: { x: number, y: number }, feature: any) => {
      ...
      if (defined(newPos) && newPos) {
          // --- MODIFICARE AICI: Normalizare pentru POINT (obiect vs array) ---
...
const handleMouseMoveGhost = (cartesian: Cartesian3, movementPosition?: { x: number, y: number }) => {
  ...
  const ghostPointPosition = new CallbackPositionProperty(() => {
      ...
      // Dacă este array (așa cum l-am forțat noi pentru editare), returnăm primul element
  ...
  const handleMapClick = (cartesian: Cartesian3) => {
      ...
      // MODIFICARE AICI: Verificăm dacă e POINT sau COMMENT pentru a salva imediat
      if (activeLayer.type === 'POINT' || activeLayer.type === 'COMMENT') {
          saveNewFeature(activeLayer, [cartesian]);
      } else {
          // Pentru LINE și POLYGON adăugăm puncte în lista temporară
...
const handleMapDoubleClick = () => {
      ...
      // MODIFICARE AICI: Ignorăm POINT și COMMENT (ele se salvează la singur click)
...
const saveNewFeature = async (layer: Layer, positions: Cartesian3[]) => {
      ...
      // MODIFICARE AICI: Logică pentru structura datelor (Object vs Array)
  ...
  const handleExportLayer = (layer: Layer) => {
      ...
      const geoJsonFeatures = layerFeatures.map(f => {
          ...
          if (layer.type === 'POINT') {
              ...
              // GeoJSON Polygons require an array of arrays (rings), usually the first one is the outer ring
              // Also, the first and last point must match. Our parser handles closing usually, but let's be safe.
              const coords = (pos as any[]).map(p => [p.longitude, p.latitude, p.height || 0]);
              if (coords.length > 0) {
                   const first = coords[0];
                   const last = coords[coords.length - 1];
                   if (first[0] !== last[0] || first[1] !== last[1]) {
                       coords.push(first); // Close the polygon loop
                   }
              }
              geometry = {
                  type: "Polygon",
  ...
  // --- FUNCȚIE IMPORT GEOJSON ---
// --- FUNCȚIE IMPORT GEOJSON (ACTUALIZATĂ - ROBUSTĂ) ---
// --- FUNCȚIE IMPORT GEOJSON (CORECATĂ PENTRU SQL SUPABASE) ---
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
      const file = event.target.files?.[0];
      if (!file) return;

      // Resetăm input-ul imediat pentru a permite re-selectarea aceluiași fișier dacă e nevoie
      ...
      reader.onload = async (e) => {
          try {
              ...
              if (!geoJson.features || !Array.isArray(geoJson.features)) {
                  throw new Error("Format invalid: Lipsește array-ul 'features'.");
              }

              setLoading(true);

              // 1. Detectare Tip Geometrie (bazat pe primul feature valid)
              const firstFeature = geoJson.features.find((f: any) => f.geometry);
              if (!firstFeature) throw new Error("Nu au fost găsite geometrii valide.");
              ...
              if (firstType === 'Point' || firstType === 'MultiPoint') layerType = 'POINT';
                  ...
                  throw new Error(`Tipul de geometrie ${firstType} nu este suportat.`);
              }

              // 2. Pregătire definiție Coloane (pentru JSONB)
              const allKeys = new Set<string>();
              geoJson.features.forEach((f: any) => {
                  if (f.properties) Object.keys(f.properties).forEach(k => allKeys.add(k));
              });
              // Ne asigurăm că 'name' este în lista de coloane vizibile
              allKeys.add('name');
              ...
              if (layerError) throw new Error(`Eroare db layers: ${layerError.message}`);
              if (!layerData) throw new Error("Layer-ul nu a putut fi creat.");
              
              const newLayerId = layerData[0].id;

              // 4. Procesare Features (Mapare 1 la 1)
              const featuresToInsert = geoJson.features.map((f: any) => {
                  const props = f.properties || {};
                  
                  // MUTĂM 'name' în PROPERTIES (pentru că nu există coloană dedicată în tabelul features)
                  if (!props.name) {
                      props.name = f.id || `Feature ${layerType}`;
                  }
                  
                  // Asigurăm extrudarea pentru poligoane
                  ...
                  // Logică extragere coordonate pentru structura simplificată a aplicației
                  if (layerType === 'POINT') {
                      // Suportă Point simplu
                      ...
                      // Suportă LineString și MultiLineString (ia prima linie)
                      ...
                      // Suportă Polygon și MultiPolygon (ia inelul exterior al primului poligon)
                      // Polygon: [ [inel_ext], [inel_int]... ] -> coords[0]
                      // MultiPolygon: [ [ [inel_ext]... ], ... ] -> coords[0][0]
                  ...
                  // Construim obiectul exact cum îl așteaptă Supabase (fără coloana 'name' la rădăcină)
                  return {
                      layer_id: newLayerId,
              ...
              // Filtrare: Eliminăm eventualele erori de geometrie
              const validFeatures = featuresToInsert.filter((f: any) => f.position_data !== null);

              if (validFeatures.length === 0) {
                   throw new Error("Nu s-au putut procesa coordonatele din fișier.");
              }
              ...
              if (featuresError) throw new Error(`Eroare db features: ${featuresError.message}`);

              // Refresh interfață
              fetchData();
              alert(`Succes! Au fost importate ${validFeatures.length} elemente în layerul "${layerName}".`);

          } catch (err: any) {
              console.error("Import failed:", err);
              alert(`Eroare la import: ${err.message}`);
              // În caz de eroare, ștergem layerul gol creat (opțional, dar recomandat)
          } finally {
              setLoading(false);
  ...
  const handleOnDragEnd = (result: DropResult) => {
    ...
    // OPTIONAL: Aici ar trebui să apelezi Supabase pentru a salva noua ordine în baza de date
    // Exemplu: await supabase.from('layers').upsert(items.map((l, index) => ({ id: l.id, order_index: index })));
  };
...
{/* HEADER STÂNGA: LOGO + TITLU (Rămâne neschimbat, la dimensiunea originală) */}
        <Paper style={{ ...glassPanelStyle, position: 'absolute', top: 20, left: 20, zIndex: 10, borderLeft: `3px solid ${COLORS.blue}`, width: 300 }} p="xs" radius="sm">
            <Group gap={10}>
                <ThemeIcon size="lg" variant="filled" color="dark" radius="sm">
                    <IconLayersIntersect size={20} color={COLORS.blue} />
                </ThemeIcon>
                <div style={{ lineHeight: 1.1 }}>
                    <Text fw={900} size="lg" style={{ fontFamily: 'Rajdhani, sans-serif' }}>
                        TERRA<span style={{ color: COLORS.blue }}>GIS</span>
                    </Text>
                    <Text size="xs" c="dimmed">{t('app.title')}</Text>
                </div>
            </Group>
        </Paper>

        {/* HEADER DREAPTA: SELECTOR LIMBĂ (Poziționat independent) */}
        <Paper style={{ ...glassPanelStyle, position: 'absolute', top: 20, right: 20, zIndex: 10 }} p={4} radius="sm">
            <SegmentedControl 
                size="xs"
                data={[
                    { label: 'EN', value: 'en' },
                ...
                styles={{ 
                    root: { backgroundColor: 'transparent' }, // Transparent pentru a se integra în glassmorphism
        ...
        <Paper style={{ ...glassPanelStyle, position: 'absolute', top: 90, left: 20, bottom: openAttributeTableId ? '40%' : 30, width: 300, zIndex: 10, display: 'flex', flexDirection: 'column' }} radius="sm">
            <Box p="sm" style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                <Group justify="space-between" mb="xs">
                    <Text size="xs" fw={700} c="dimmed">LAYERE ({layers.length})</Text>
                    <Group gap={5}>
                        {/* INPUT ASCUNS PENTRU IMPORT */}
                        <input 
                            type="file" 
                            accept=".geojson,.json" 
                            ref={fileInputRef} 
...
<ScrollArea style={{ flex: 1 }} p="xs">
                                                ...
                                                >
                                                    ...
                                                    >
                                                        <Group justify="space-between" mb={4}>
                                                            <Group gap={8}>
                                                                {/* HANDLE PENTRU DRAG (zona de care tragi) */}
                                                                ...
                                                                {layer.type === 'COMMENT' && <IconMessageCircle size={16} color="#ffffff" style={{ opacity: 0.8 }} />}
                                                            ...
                                                            <Group gap={2}>
                                                                <ActionIcon 
                                                                    ...
                                                                    variant="subtle" 
                                                                    color="green" 
                                                                    title="Exportă GeoJSON"
                                                                    onClick={(e) => { e.stopPropagation(); handleExportLayer(layer); }}
                                                                >
                                                        ...
                                                        {/* Restul interfeței layer-ului (Buton tabel, Legendă) rămâne la fel... */}
                                                                        ...
                                                                        return (
                                                                            <Group key={val} justify="space-between" gap={8}>
                                                                                ...
                                                                                <ColorInput 
                                                                                    ...
                                                                                    styles={{ 
                                                                                        input: { 
                                                                                            width: 100, height: 26, minHeight: 26, padding: 0, paddingLeft: 30, fontSize: 11, color: 'white', backgroundColor: 'rgba(255,255,255,0.05)', borderColor: 'rgba(255,255,255,0.1)', cursor: 'pointer'
                                                                                        }, 
                                                                                        swatch: { width: 18, height: 18, marginLeft: 4, cursor: 'pointer' } 
                                                                                    }}
                                                                                />
                                                                            </Group>
                                                                        );
        ...
        <Modal opened={!!tempLayerSettings} onClose={() => setTempLayerSettings(null)} title="Setări Layer" centered styles={{ content: { backgroundColor: '#1A1B1E', color: 'white' }, header: { backgroundColor: '#1A1B1E', color: 'white' } }}>
            {tempLayerSettings && (
            <Stack>
                <TextInput label="Nume Layer" value={tempLayerSettings.name} disabled styles={{ input: { backgroundColor: '#2C2E33', color: '#999' } }}/>
                
                {tempLayerSettings.type === 'COMMENT' ? (
                    <div style={{ padding: 10, backgroundColor: 'rgba(220, 38, 38, 0.15)', border: '1px solid #ef4444', borderRadius: 4 }}>
                         <Text size="sm" c="red.4" fw={500}>
                             Stilizarea este blocată pentru comentarii. 
                             Acestea sunt afișate standard ca semnul exclamării (!).
                         </Text>
                    </div>
                ) : (
                    <>
                        ...
                        {(!tempLayerSettings.style_props.visType || tempLayerSettings.style_props.visType === 'single') ? (
                            ...
                            <Select
                                ...
                                onChange={(val) => setTempLayerSettings({ ...tempLayerSettings, style_props: { ...tempLayerSettings.style_props, visColumn: val || undefined } })}
                            />
                        )}
                        {tempLayerSettings.type !== 'POLYGON' && <NumberInput label="Grosime / Mărime (px)" value={tempLayerSettings.type === 'POINT' ? tempLayerSettings.style_props.pixelSize : tempLayerSettings.style_props.width} onChange={(val) => { const key = tempLayerSettings.type === 'POINT' ? 'pixelSize' : 'width'; setTempLayerSettings({ ...tempLayerSettings, style_props: { ...tempLayerSettings.style_props, [key]: Number(val) } }) }} />}
                        {tempLayerSettings.type === 'POLYGON' && <NumberInput label="Extrudare (Volum 3D)" value={tempLayerSettings.style_props.extrudedHeight} onChange={(val) => setTempLayerSettings({ ...tempLayerSettings, style_props: { ...tempLayerSettings.style_props, extrudedHeight: Number(val) } })} />}
                    </>
                )}

                <Divider my="sm" />
                <Group justify="flex-end"><Button variant="default" onClick={() => setTempLayerSettings(null)}>Anulează</Button><Button color="blue" onClick={saveLayerSettings} leftSection={<IconDeviceFloppy size={16}/>}>Salvează Modificările</Button></Group>
            </Stack>)}
        </Modal>
        ...
        <Modal opened={showNewLayerModal} onClose={() => setShowNewLayerModal(false)} title="Layer Nou" centered styles={{ content: { backgroundColor: '#1A1B1E', color: 'white' }, header: { backgroundColor: '#1A1B1E', color: 'white' } }}>
            <Stack>
                <TextInput label="Nume Layer" value={newLayerName} onChange={(e) => setNewLayerName(e.target.value)} styles={{ input: { backgroundColor: '#2C2E33', color: 'white' } }}/>
                <Select 
                    label="Tip Geometrie" 
                    data={['POINT', 'LINE', 'POLYGON', 'COMMENT']} 
                    value={newLayerType} 
                    onChange={setNewLayerType} 
                    styles={{ input: { backgroundColor: '#2C2E33', color: 'white' } }} 
                />
                <Button fullWidth onClick={handleCreateLayer} color="blue">Creează</Button>
            </Stack>
        </Modal>

        {/* PANOUL PENTRU EDITARE POZIȚIE (MOVE) */}
        {isRelocating && (
        ...
        {/* PANOUL PENTRU EDITARE VERTEXI (NOU) */}
        {isEditingVertices && (
            <Paper style={{ ...glassPanelStyle, position: 'absolute', top: 20, left: '50%', transform: 'translateX(-50%)', zIndex: 20, borderColor: COLORS.orange }} p="md" radius="sm">
                <Group>
                    <IconPolygon size={24} color={COLORS.orange} />
                    <div>
                        <Text fw={700} c="white">MOD EDITARE VERTEXI</Text>
                        <Text size="xs" c="dimmed">{activeVertexIndex !== null ? "Mută punctul și dă click pentru a fixa." : "Selectează un punct alb (vertex) pentru a-l muta."}</Text>
                    </div>
                    <Button size="xs" color={isSnappingEnabled ? "cyan" : "gray"} variant={isSnappingEnabled ? "filled" : "outline"} leftSection={<IconMagnet size={16} />} onClick={() => setIsSnappingEnabled(!isSnappingEnabled)}>
                        {isSnappingEnabled ? "Snap On" : "Snap Off"}
                    </Button>
                    <Button size="xs" color="red" variant="light" onClick={() => { setIsEditingVertices(false); setActiveVertexIndex(null); ghostPositionRef.current = null; }}>
                        Termină
                    </Button>
                </Group>
            </Paper>
...
{selectedFeatureId && (
             <div style={{ position: 'absolute', top: 90, right: 20, zIndex: 15, display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                 
                 {/* AM ȘTERS BLOCUL CU BUTONUL "Editează Vertexi" DE AICI */}
        ...
        >
             ...
             {/* --- POZIȚIONARE CAMERĂ INIȚIALĂ --- */}
            <CameraFlyTo 
                destination={Cartesian3.fromDegrees(26.056986, 44.442540, 350)}
                orientation={{
                    heading: CesiumMath.toRadians(800),   // 0 = Nord
                    pitch: CesiumMath.toRadians(-55),   // -35 grade (o vedere oblică, bună pentru 3D)
                    roll: 0.0
                }}
                duration={0}
                once={true}
             ...
             <MapEvents 
                ...
                onVertexSelect={(idx) => {
                    if (isEditingVertices) {
                        ...
                        const feature = features.find(f => f.id === selectedFeatureId);
                        if (feature) {
                             // --- MODIFICARE AICI: Normalizare array/object ---
                             let positions: Cartesian3[] = [];
                             if (Array.isArray(feature.position_data)) {
                                 positions = feature.position_data.map((p: any) => 
                                    Cartesian3.fromDegrees(p.longitude, p.latitude, p.height)
                                 );
                             } else {
                                 const p = feature.position_data;
...
{/* RENDER STATIC - FIXAT PENTRU DRAG & DROP */}
                ...
                return (
                    <Fragment key={layer.id}>
                        {layerFeatures.map(feature => {
                            // Dacă relocăm sau edităm vertexi, nu randăm originalul pentru feature-ul selectat
                            ...
                            if (layer.type === 'COMMENT') {
                                ...
                                return <Entity 
                                    ...
                                    label={{
                                        ...
                                        eyeOffset: new Cartesian3(0, 0, 0) // Îl aducem puțin în față
                                    }}
                                />;
                            }
...
{/* VERTEX HANDLES (PUNCTE ALBE) - Vizibile doar în modul Editare Vertexi */}
             {isEditingVertices && selectedFeatureId && (() => {
                 const feature = features.find(f => f.id === selectedFeatureId);
                 if (!feature || feature.layer_id === undefined) return null;
                 const layer = layers.find(l => l.id === feature.layer_id);
                 
                 // --- MODIFICARE AICI: Am scos condiția "|| layer.type === 'POINT'" ---
                 if (!layer) return null; 

                 // --- MODIFICARE AICI: Normalizare date ---
                 let positions: Cartesian3[] = [];
                 if (Array.isArray(feature.position_data)) {
                 ...
                 return positions.map((pos, idx) => (
                     <Entity 
                        key={`handle_${idx}`}
                        id={`vertex_handle_${idx}`} 
                        position={pos}
                        show={activeVertexIndex !== idx} 
                        point={{
                            pixelSize: activeVertexIndex === idx ? 15 : 10, 
                            color: activeVertexIndex === idx ? Color.RED : Color.WHITE,
             ...
             {/* GHOST RENDER (Relocare + Vertex Edit) */}
                 ...
                 (() => {
                     ...
                     if (layer.type === 'POINT' || layer.type === 'COMMENT') {
                         ...
                         return <Entity polygon={{ 
                             ...
                             outlineColor: Color.WHITE, 
                             outlineWidth: 3, 
                             extrudedHeight: isExtruded ? new CallbackProperty(() => { const pos = ghostPositionRef.current; if(!pos || !pos[0]) return undefined; const c = Cartographic.fromCartesian(pos[0]); return c.height + extrusionValue; }, false) : undefined, 
                             perPositionHeight: true, 
                             classificationType: undefined 
                         }} />;
                     }
